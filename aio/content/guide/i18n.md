...
* [Set the source locale manually](#set-source-manually) if you need to set the [LOCALE_ID](api/core/LOCALE_ID "API reference for LOCALE_ID") token.
  ...
  如果你需要设置 [LOCALE_ID](https://angular.cn/api/core/LOCALE_ID "LOCALE_ID 的 API 参考") 令牌，请[手动设置源本地环境](#set-source-manually) 。
...
关于 `package.json` 和 polyfill 包的更多信息，请参阅[工作区的 npm 依赖项](https://angular.io/guide/npm-packages) 。
...
[Angular 的代码仓库中](https://github.com/angular/angular/tree/master/packages/common/locales "Angular 存储库中的常用语言环节")包含常见的本地环境。你可以通过在应用的[工作区配置](guide/workspace-config "Angular 工作区配置")文件 `angular.json` 中的 `sourceLocale` 字段中设置源本地环境来为此构建更改应用的源本地环境。构建过程（在本指南的[把翻译合并到应用程序](#merge)中描述）会使用应用中的 `angular.json` 文件自动设置 [`LOCALE_ID`](api/core/LOCALE_ID "LOCALE_ID 的 API 参考") 令牌并加载本地化数据。
...
注意：`--i18n-locale` 选项已弃用。 Angular 9 会使用应用的[工作区配置](guide/workspace-config "Angular 工作区配置")文件（ `angular.json` ）中[配置](guide/workspace-config "Angular 工作区配置")的源本地环境。
...
要为每个本地环境构建应用的一个可分发副本，请在项目工作区配置文件 [`angular.json`](guide/workspace-config "Angular 工作区配置") 中的[构建配置](#localize-config)中[定义本地环境](#localize-config)。这个方法通过放弃为每个本地环境执行完整构建的需求，缩短了构建过程。
...
在应用的构建配置（ [`angular.json`](guide/workspace-config "Angular 工作区配置") ）中使用 `i18n` 项目选项来定义项目的本地环境。它下面的子选项用于标识源语言，并告诉编译器要在哪里找到该项目支持的各种翻译：
...
* [Set the source locale manually](#set-source-manually) by setting the [LOCALE_ID](api/core/LOCALE_ID "API reference for LOCALE_ID") token.
  ...
  通过设置 [LOCALE_ID](https://angular.cn/api/core/LOCALE_ID "LOCALE_ID 的 API 参考") 令牌来[手动设置源本地环境](#set-source-manually) 。
* [Import global variants of the locale data](#import-locale) for extra locale data.
